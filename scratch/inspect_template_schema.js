...
async function main() {
  ...
  const tRes = await axios.get(`${apiBaseUrl}/api/templates`, {
    headers: { Authorization: `Bearer ${token}`, "x-api-key": partnerKey },
    timeout: 10000
  });

  const list = tRes.data?.data?.templates || tRes.data?.templates || [];
  const aiAssistant = list.find(t => t.name === "ai_assistant" || t.templateName === "ai_assistant");
  console.log("Template ai_assistant:", JSON.stringify(aiAssistant, null, 2));
}
