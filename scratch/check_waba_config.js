...
async function main() {
  ...
  try {
    const wRes = await axios.get(`${apiBaseUrl}/api/waba`, {
      headers: { Authorization: `Bearer ${token}`, "x-api-key": partnerKey, "x-client-id": lakshmiClientId }
    });
    console.log("WABA config:", JSON.stringify(wRes.data, null, 2));
  } catch (e) {
    console.log("WABA fetch error:", e.response?.data || e.message);
  }
}
