...
async function main() {
  ...
  try {
    const res = await axios.get(`${apiBaseUrl}/api/campaigns`, { headers });
    const list = res.data?.data?.campaigns || [];
    console.log("Found campaigns on Whats AI:", list.length);
    list.slice(0, 5).forEach(c => {
      console.log(`- ID: ${c._id || c.id} | Name: ${c.name} | userId: ${c.userId}`);
    });
  } catch (e) {
