...
async function main() {
  ...
  console.log("=== 1. Fetch Conversations from Whats AI ===");
  ...
  for (const c of convs.slice(0, 6)) {
    console.log(`Contact: ${c.contactName || c.phone}, Phone: ${c.phone}, Last Msg: ${c.lastMessage}, Status: ${c.status}, Unread: ${c.unreadCount}`);
    
    // Fetch last messages in this conversation
    try {
      const mRes = await axios.get(`${apiBaseUrl}/api/inbox/conversations/${c._id || c.id}/messages`, { headers, timeout: 10000 });
      const msgs = mRes.data?.data?.messages || mRes.data?.messages || [];
      console.log(`  Last ${Math.min(msgs.length, 3)} messages:`);
      msgs.slice(-3).forEach(m => {
        console.log(`  - [${m.status}] ${m.direction} | ${m.body || m.text} | errorReason: ${m.errorReason || 'None'} | id: ${m.whatsappMessageId || m._id}`);
      });
    } catch (e) {
      console.log("  Error fetching msgs:", e.message);
    }
    console.log("---");
  }
}
