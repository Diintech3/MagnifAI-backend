...
async function checkRajStatus() {
  ...
  const res = await axios.get(`${apiBaseUrl}/api/inbox/conversations/6a844da75bccf706d7b7d84b/messages`, {
    headers: { Authorization: `Bearer ${token}`, "x-api-key": partnerKey, "x-client-id": lakshmiClientId },
    timeout: 10000
  });

  const msgs = res.data?.data?.messages || [];
  const target = msgs.find(m => m.whatsappMessageId === "wamid.HBgMOTE4NzI2NTI1NzgyFQIAERgSRDZDQjg2QkY0RDRFMzI0Rjg1AA==");
  console.log("Raj message status:", JSON.stringify(target, null, 2));
}

checkRajStatus().catch(console.error);
