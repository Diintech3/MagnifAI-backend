...
async function main() {
  ...
  console.log("=== Test 1: PATCH /api/contacts/" + anandId + " with clientHeaders and { group: ['Akkash'] } ===");
  try {
    ...
    console.log("Err 1:", e.response?.data || e.message);
  }

  console.log("=== Test 2: PATCH /api/contacts/" + anandId + " with clientHeaders and { name: 'anand' } ===");
  try {
    const res = await axios.patch(
      `${apiBaseUrl}/api/contacts/${anandId}`,
      { name: "anand", group: [akkashGroupId] },
      { headers: clientHeaders }
    );
    console.log("Result 2:", res.data);
  } catch (e) {
    console.log("Err 2:", e.response?.data || e.message);
  }
}
