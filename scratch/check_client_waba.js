...
async function main() {
  ...
  try {
    const pRes = await axios.get(`${apiBaseUrl}/api/clients/${lakshmiClientId}`, {
      headers: { Authorization: `Bearer ${token}`, "x-api-key": partnerKey }
    });
    console.log("Client WABA info:", JSON.stringify(pRes.data, null, 2));
  } catch (e) {
    console.log("Client fetch error:", e.response?.data || e.message);
  }
}
