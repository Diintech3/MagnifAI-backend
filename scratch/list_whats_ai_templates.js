...
async function main() {
  ...
  const list = tRes.data?.data?.templates || tRes.data?.templates || tRes.data || [];
  console.log("All templates count:", list.length);
  list.forEach(t => console.log(`- ${t.name || t.templateName} (${t.status}) - ID: ${t._id || t.id}`));
}
