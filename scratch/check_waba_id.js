...
async function checkWabaId() {
  ...
  try {
    const res = await axios.get(`${apiBaseUrl}/api/settings/whatsapp`, {
      headers: { Authorization: `Bearer ${token}`, "x-api-key": partnerKey, "x-client-id": lakshmiClientId }
    });
    console.log("Settings whatsapp:", res.data);
  } catch (e) {
    console.log("Settings error:", e.response?.data || e.message);
  }
}

checkWabaId().catch(console.error);
