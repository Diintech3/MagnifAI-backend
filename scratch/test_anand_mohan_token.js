...
async function testAnandMohanRoutes() {
  try {
    await mongoose.connect(env.MONGODB_URI);
    console.log("Connected to DB successfully!");

    const ceo = await CEO.findOne({ email: "anandmoha.7970@gmail.com" });
    if (!ceo) {
      console.error("CEO Anand Mohan not found!");
    ...
    console.log("Generated Anand Mohan CEO Access Token:", token);
    ...
    console.error("Error in testAnandMohanRoutes:", err);
  }
}

testAnandMohanRoutes();
